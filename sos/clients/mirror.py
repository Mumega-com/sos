...
class MirrorClient:
    ...
    async def store_experience(self, engram: Dict) -> Optional[str]:
        """Store a Hermes Experience Ledger engram (S064 Track A).

        engram must match the ExperienceEngramRequest schema:
            agent_id, tenant_id, project_id, action_type,
            context_snapshot {target_industry, channel, hook_style},
            outcome {success, metric_value, feedback_signal},
            the_lesson, citable_proof (optional)

        Returns engram_id on success, None on failure.
        """
        try:
            resp = await self._request("POST", "/experience/store", json=engram)
            if resp.status_code == 200:
                data = resp.json()
                log.info(f"✓ Experience engram stored: {data.get('engram_id')}")
                return data.get("engram_id")
            log.error(f"❌ store_experience failed: {resp.status_code} {resp.text}")
            return None
        except Exception as e:
            log.error(f"store_experience error: {e}")
            return None
    ...
    async def recall_experience(
        ...
        """Recall top experience engrams by action_type + target_industry.

        Usage:
            engrams = await client.recall_experience("outreach", "dental")
        """
        try:
            payload = {
            ...
            resp = await self._request("POST", "/experience/recall", json=payload)
            if resp.status_code == 200:
                return resp.json().get("engrams", [])
            log.error(f"❌ recall_experience failed: {resp.status_code} {resp.text}")
            return []
        except Exception as e:
            log.error(f"recall_experience error: {e}")
            return []
