...
The current sprint slug is stored in SOS_HOME/.current_sprint. Sprint markers
emit to audit_events with stream_id='kernel' and action='sprint_marker'. Roll-up
is a SQL query bounded by start/close timestamps.
...
SOS_REPO = Path(os.getenv("SOS_HOME", str(Path.home() / "SOS")))
SITE_REPO = Path(os.getenv("SOS_SITE_REPO", str(Path.home() / "site")))
MIRROR_REPO = Path(os.getenv("SOS_MIRROR_REPO", str(Path.home() / "mirror")))
CURRENT_SPRINT_FILE = Path(os.getenv("SOS_CURRENT_SPRINT_FILE", str(SOS_REPO / ".current_sprint")))
...
class SprintStats:
    ...
    commits_landed: int  # across SOS, site repo, mirror
...
def _count_commits_in_window(start: str | None, end: str | None) -> int:
    if not (start and end):
        return 0
    total = 0
    for repo in (SOS_REPO, SITE_REPO, MIRROR_REPO):
        try:
            out = subprocess.check_output(
