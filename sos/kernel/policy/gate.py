...
from sos.kernel.auth import AuthContext, has_scope, verify_bearer
...
def _check_tenant_scope(ctx: AuthContext, tenant: str) -> tuple[bool, str]:
    """Return (ok, reason) — admin is global; scoped services are explicit.
    ...
    """
    if ctx.is_admin or has_scope(ctx, "*"):
        return True, "system/admin scope"
    if ctx.is_system and ctx.scopes:
        return True, f"service scope {','.join(ctx.scopes)}"
...
async def can_execute(
    *,
    ...
    if auth_ctx is not None:
        ...
        # Elevated, scoped-service, and tenant callers are tracked separately so
        # audit records show why the caller got in.
        if auth_ctx.is_admin or has_scope(auth_ctx, "*"):
            pillars_passed.append("system_admin")
        elif auth_ctx.is_system:
            pillars_passed.append("service_scope")
        else:
