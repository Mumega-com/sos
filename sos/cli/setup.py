"""
SOS dev-machine interactive setup — run with: python -m sos.cli.setup

Configures LLM provider, API keys, bus tokens, and Redis for a single
developer workstation. This is the old sos/cli/init.py wizard, renamed
when v0.9.4 repurposed ``init`` for tenant provisioning (Phase 5).
...
from sos.bus.token_store import hash_token, load_tokens, write_tokens_atomic
...
def write_tokens(admin_token: str, agent_token: str) -> None:
    ...
    if new_tokens:
        ...
        write_tokens_atomic(TOKENS_FILE, existing)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="SOS dev-machine setup")
    parser.add_argument(
        "--defaults",
        action="store_true",
