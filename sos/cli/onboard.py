...
def create_bus_token(slug: str) -> str:
    ...
    created, record = append_if_missing(TOKENS_FILE, {
        "token": token,
        ...
        "scope": "customer",
        "scopes": ["bus:send"],
    }, lambda t: t.get("project") == slug)
    if not created:
        existing = record.get("token", "")
        print(f"  [3/8] Bus token exists: {existing[:20]}...")
        return existing
    ...
    settings = {
        "hooks": {
            ...
            "Stop": [{
                ...
                "hooks": [{
                    "type": "command",
                    "command": f"AGENT_NAME={slug} /home/sos/.claude/hooks/session-tracker.sh",
                    "timeout": 10,
                }],
            }],
        },
