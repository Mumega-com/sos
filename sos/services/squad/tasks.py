...
from sos.services.squad.event_router import route_task_event
...
DISCORD_WEBHOOK_TIMEOUT_SECONDS = float(os.getenv("SQUAD_DISCORD_WEBHOOK_TIMEOUT_SECONDS", "3"))
...
def _emit_task_completed(task: SquadTask, actor: str, completed_at: str) -> None:
    ...
    if task.bounty and task.bounty.get("bounty_id"):
        ...
        # S063 Track D: propagate capability_grant so the QNFT bridge can pick it up
        if task.bounty.get("capability_grant"):
            result_extras["capability_grant"] = task.bounty["capability_grant"]
    if task.project:
...
def _safe_discord_text(value: Any, limit: int = 900) -> str:
...
def build_discord_task_payload(event_type: str, task: SquadTask, actor: str) -> dict[str, Any]:
    """Return a Discord payload containing only public task fields.

    The projection deliberately omits auth headers, token hashes, tenant IDs,
    inputs/result blobs, and bounty internals. Discord is a human visibility
    surface, not a replica of the task record.
    """
    action = "Created" if event_type == "task.created" else "Updated"
    fields = [
        {"name": "ID", "value": _safe_discord_text(task.id, 120), "inline": True},
        {"name": "Priority", "value": _safe_discord_text(task.priority.value, 80), "inline": True},
        {"name": "Actor", "value": _safe_discord_text(actor, 120), "inline": True},
        {"name": "Status", "value": _safe_discord_text(task.status.value, 80), "inline": True},
        {"name": "Project", "value": _safe_discord_text(task.project or "unknown", 120), "inline": True},
    ]
    if task.assignee:
        fields.append({"name": "Assignee", "value": _safe_discord_text(task.assignee, 120), "inline": True})
    return {
        "content": f"[mumega] Task {action}: {_safe_discord_text(task.title, 160)}",
        "embeds": [
            {
                "title": _safe_discord_text(task.title, 256),
                "description": _safe_discord_text(task.description, 900),
                "fields": fields,
            }
        ],
    }
...
def _project_webhook_env_name(project: str) -> str:
    slug = re.sub(r"[^A-Z0-9]+", "_", project.upper()).strip("_")
    return f"SQUAD_DISCORD_WEBHOOK_URL_{slug}" if slug else ""


def discord_webhook_for_project(project: str | None) -> str:
    project = (project or "").strip()
    raw_mapping = os.getenv("SQUAD_DISCORD_WEBHOOKS_JSON", "").strip()
    if raw_mapping:
        try:
        ...
        if isinstance(mapping, dict):
            specific = mapping.get(project)
            fallback = mapping.get("sos-global") or mapping.get("global") or mapping.get("*")
            selected = specific or fallback
            if selected:
                return str(selected).strip()
    env_name = _project_webhook_env_name(project)
    if env_name and os.getenv(env_name):
        return str(os.getenv(env_name, "")).strip()
    return (
        os.getenv("SQUAD_DISCORD_WEBHOOK_URL")
        or os.getenv("SOS_DISCORD_WEBHOOK_URL")
        or os.getenv("DISCORD_WEBHOOK_URL")
        or ""
    ).strip()


def project_task_to_discord(event_type: str, task: SquadTask, actor: str) -> None:
    webhook_url = discord_webhook_for_project(task.project)
    if not webhook_url:
        return
    payload = build_discord_task_payload(event_type, task, actor)
    ...
    def _send() -> None:
        try:
            ...
            with urllib.request.urlopen(request, timeout=DISCORD_WEBHOOK_TIMEOUT_SECONDS):
                pass
        except Exception as exc:
            log.warn(
                "discord task projection failed",
                task_id=task.id,
...
def row_to_task(row: sqlite3.Row) -> SquadTask:
    ...
    return SquadTask(
        ...
        decision_required=bool(row["decision_required"]) if "decision_required" in row.keys() else False,
        options=_loads(row["options_json"], []) if "options_json" in row.keys() else [],
...
class SquadTaskService:
        ...
        with self.db.connect() as conn:
            conn.execute(
                ...
                INSERT INTO squad_tasks (
                    ...
                    labels_json, blocked_by_json, blocks_json, inputs_json, result_json, decision_required, options_json, token_budget,
                ...
                (
                    ...
                    _dumps(task.inputs),
                    _dumps(task.result),
                    1 if task.decision_required else 0,
                    _dumps(task.options),
                    task.token_budget,
                    _dumps(task.bounty),
                    task.external_ref,
    ...
    def complete(
        ...
        route_task_event(
            ...
            bus=self.bus,
            tenant_id=tenant_id if tenant_id is not None else DEFAULT_TENANT_ID,
            squad_payload={"task_id": task.id, "result": result},
            notion=False,
        )
    ...
    def fail(
        ...
        route_task_event(
            ...
            bus=self.bus,
            tenant_id=tenant_id if tenant_id is not None else DEFAULT_TENANT_ID,
            squad_payload={"task_id": task.id, "error": error},
            notion=False,
        )
        return task

    def apply_notion_decision(
        self,
        task_id: str,
        *,
        decision_status: str,
        ...
        if not task.decision_required:
            raise ValueError(f"Task {task_id} is not a decision task")

        decision_status = (decision_status or "").strip()
        selected_decision = (selected_decision or "").strip()
        timestamp = now_iso()
        result = dict(task.result or {})
        result["notion_decision"] = {
            "page_id": page_id,
        ...
        status_key = decision_status.lower().replace(" ", "_")
        if selected_decision or status_key == "done":
            task.status = TaskStatus.DONE
