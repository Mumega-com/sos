...
async def health() -> JSONResponse:
    ...
    try:
        def _ping() -> None:
            c = _sqlite3.connect(str(DB_PATH), timeout=1.0)
            c.execute("SELECT 1")
            c.close()
...
_SYSTEM_BEARERS = {
    token
    for token in (
        os.getenv("SOS_SQUAD_SYSTEM_TOKEN", ""),
        os.getenv("SOS_SQUAD_TOKEN", ""),
        os.getenv("SOS_SYSTEM_TOKEN", ""),
    )
    if token
}
...
def _require_system_bearer(authorization: Optional[str]) -> None:
    ...
    if presented not in _SYSTEM_BEARERS:
...
async def auth_verify(
    ...
    Admin-gated: only callers presenting a Squad service bearer can verify tokens
...
async def _stale_claim_reaper_loop() -> None:
    ...
    # Startup sweep — run in executor so it never blocks the event loop
    try:
        reset = await loop.run_in_executor(None, lambda: tasks.reap_stale_claims(tenant_id=None))
        if reset:
            _log.info("squad.startup_reap: reset %d stale claimed task(s)", reset)
    except Exception as exc:
        _log.warning("squad.startup_reap failed (non-fatal): %s", exc)
    # Periodic sweep — always offloaded to thread pool
    while True:
...
_SQUAD_BUS_TOKEN: str = os.environ.get("SOS_SQUAD_TOKEN", "")
