"""GET /marketplace, GET /marketplace/skill/{id}, POST /api/marketplace/publish."""
...
# ---------------------------------------------------------------------------
# POST /api/marketplace/publish — S063 Slice 3
#
# Auth: Bearer token with QNFT capability marketplace:publish ("Vibe Coder").
# Body: JSON App Manifest.
# Storage: marketplace_listings (local D1-equivalent SQLite).
# ---------------------------------------------------------------------------

_REQUIRED_MANIFEST_FIELDS = {"name", "description", "category", "listing_type", "price_cents"}
_ALLOWED_LISTING_TYPES = {"squad", "skill", "agent", "workflow"}
_ALLOWED_CATEGORIES = {"seo", "content", "outreach", "research", "ops", "data", "custom"}


def _get_redis() -> redis.Redis:
    import os
    pw = os.environ.get("REDIS_PASSWORD", "")
    url = f"redis://:{pw}@localhost:6379/0" if pw else "redis://localhost:6379/0"
    return redis.Redis.from_url(url, decode_responses=True)


def _has_marketplace_publish_capability(agent: str, tenant: str) -> bool:
    """Check QNFT Redis set for marketplace:publish capability."""
    try:
        r = _get_redis()
        return bool(r.sismember(f"qnft:capabilities:{tenant}:{agent}", "marketplace:publish"))
    except Exception:
        return False
...
def _tenant_is_active(tenant_slug: str) -> bool:
    """Return False if tenant is in Stasis (is_active=0). Default True."""
    try:
        conn = _get_db()
        row = conn.execute(
        ...
        if row is None:
            return True  # unknown tenant — let it through, not our gate
        return bool(row["is_active"])
    except Exception:
        return True  # fail-open for missing column during migration window


@router.post("/api/marketplace/publish")
async def publish_manifest(request: Request) -> JSONResponse:
    """Publish a JSON App Manifest to the marketplace catalog.

    Auth: Bearer token; agent must hold QNFT capability marketplace:publish.
    Tenant must be active (is_active=1); returns 403 tenant_in_stasis otherwise.
    """
    # --- Auth ---
    ...
    ctx = _verify(f"Bearer {raw_token}")
    if ctx is None:
        return JSONResponse({"error": "unauthorized", "message": "Invalid token"}, status_code=401)

    agent = ctx.agent or ""
    tenant_slug = ctx.tenant_slug or ctx.project or ""

    # --- Tenant stasis gate (FSD hard-block) ---
    if tenant_slug and not _tenant_is_active(tenant_slug):
        return JSONResponse(
            {"error": "tenant_in_stasis", "message": "Tenant is in Stasis — publish blocked"},
            status_code=403,
        )

    # --- Vibe Coder capability check ---
    if not _has_marketplace_publish_capability(agent, tenant_slug):
        return JSONResponse(
            {
                "error": "forbidden",
                "message": f"Agent '{agent}' does not hold QNFT capability marketplace:publish",
                "hint": "Complete a marketplace:publish bounty to unlock this capability",
            },
            status_code=403,
        )
    ...
    # --- Parse manifest ---
    try:
        manifest: dict[str, Any] = await request.json()
    except Exception:
    ...
    if missing:
        return JSONResponse(
            {"error": "invalid_manifest", "message": f"Missing required fields: {sorted(missing)}"},
            status_code=422,
        )

    listing_type = str(manifest.get("listing_type", ""))
    if listing_type not in _ALLOWED_LISTING_TYPES:
        return JSONResponse(
            {"error": "invalid_manifest", "message": f"listing_type must be one of {sorted(_ALLOWED_LISTING_TYPES)}"},
            status_code=422,
        )
    ...
    price_cents = manifest.get("price_cents", 0)
    if not isinstance(price_cents, int) or price_cents < 0:
        return JSONResponse({"error": "invalid_manifest", "message": "price_cents must be a non-negative integer"}, status_code=422)

    # --- Write to marketplace_listings (D1 / SQLite) ---
    listing_id = f"lst-{uuid4().hex[:12]}"
    ...
    try:
        ...
        conn.execute(
            """INSERT INTO marketplace_listings
               (id, seller_tenant, title, description, category, listing_type,
                price_cents, price_model, squad_template, skill_ids, tags,
                active, subscriber_count, rating, created_at, updated_at)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1, 0, 0.0, ?, ?)""",
            (
                listing_id,
                tenant_slug or agent,
                str(manifest.get("name", ""))[:200],
                str(manifest.get("description", ""))[:2000],
                category,
                listing_type,
                price_cents,
                str(manifest.get("price_model", "monthly")),
                _json.dumps(manifest.get("squad_template")) if manifest.get("squad_template") else None,
                _json.dumps(manifest.get("skill_ids", [])),
                _json.dumps(manifest.get("tags", [])),
                now,
                now,
            ),
        )
        ...
        logger.exception("marketplace publish DB write failed")
        return JSONResponse({"error": "internal", "message": str(exc)}, status_code=500)

    logger.info("marketplace publish listing_id=%s agent=%s tenant=%s", listing_id, agent, tenant_slug)
    return JSONResponse(
        {
            "ok": True,
            "listing_id": listing_id,
            "status": "published",
            "url": f"/marketplace/listing/{listing_id}",
            "manifest": {"name": manifest.get("name"), "category": category, "price_cents": price_cents},
        },
        status_code=201,
    )
