...
class SOSEngine(EngineContract):
    ...
    async def _deep_dream_synthesis(self):
        ...
        try:
            ...
            # Select model via resilient router to avoid retrying a dead adapter.
            # `preferred_model` is optional; if invalid/misconfigured, the router
            # falls back to the chain (including Vertex, Grok, OpenAI, local).
            preferred_model = os.getenv("SOS_DREAM_MODEL", "")
            preferred_model = preferred_model if preferred_model in self.models else ""
            if preferred_model:
                log.info(f"Dreaming via preferred model: {preferred_model}...")
            else:
                log.info("Dreaming via resilient fallback chain (default ordering)...")

            insight, dream_model = await self.router.generate(
                prompt=dream_prompt,
