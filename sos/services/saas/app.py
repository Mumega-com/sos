...
from sos.bus.token_store import append_record, hash_token, load_tokens, update_matching
...
from sos.services.saas.onboard_routes import router as onboard_router
...
def _revoke_seat(slug: str, token_id: str) -> None:
    ...
    def _matches(t: dict) -> bool:
        if t.get("project") == slug and t.get("scope") == "customer":
            token_hash = t.get("token_hash", "") or hashlib.sha256(t.get("token", "").encode()).hexdigest()
            return token_hash.endswith(token_id)
        return False

    changed = update_matching(tokens_path, _matches, lambda t: t.__setitem__("active", False))
    if changed:
...
def _get_squad_service_header() -> dict[str, str]:
    ...
    key = os.environ.get("SOS_SQUAD_TOKEN", "")
