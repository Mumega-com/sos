...
def _squad_service_token() -> str:
    return (
        os.environ.get("SOS_HEALTH_TOKEN")
...
def check_squad_service() -> dict:
    # Check both instances directly — only report down if BOTH fail.
    # Avoids false-positive restarts when nginx proxy times out but a backend is healthy.
    primary_url = "http://127.0.0.1:18060/health"
    secondary_url = "http://127.0.0.1:18061/health"
    errors = []
    for url in (primary_url, secondary_url):
        try:
            resp = requests.get(url, timeout=5)
            if resp.status_code == 200:
                return {"status": "ok", "instance": url}
            errors.append(f"{url}: http {resp.status_code}")
        except Exception as e:
            errors.append(f"{url}: {e}")
    return {"status": "down", "error": "; ".join(errors)}
    ...
    for agent_id in get_executor_agents():
        try:
            resp = requests.get(
                ...
                headers={"Authorization": f"Bearer {_squad_service_token()}"},
                timeout=5,
            )
            if resp.status_code != 200:
                continue
