...
States detected:
  - dead: tmux session gone or OpenClaw session expired
  - stuck: no output change for 30 minutes with in_progress tasks
...
STUCK_THRESHOLD_MINUTES = int(os.environ.get("LIFECYCLE_STUCK_MINUTES", "30"))
...
def _snapshot_agent_context(agent_id: str, state: dict, agent_def: dict) -> None:
    ...
    if r:
        try:
            ...
            if messages:
                state["recent_bus_messages"] = [
                    m[1].get("data", m[1].get("text", ""))[:200]
                    for m in messages
                ]
        except Exception:
            pass
...
def detect_agent_state(agent_id: str, agent_def: dict) -> dict:
    ...
    Returns: {"state": "dead"|"stuck"|"compacted"|"busy"|"idle", "details": str}
    """
    if agent_def.get("type") == "openclaw":
        return _detect_openclaw_state(agent_id)
...
def _detect_openclaw_state(agent_id: str) -> dict:
...
def handle_compacted_agent(agent_id: str, agent_def: dict) -> bool:
    """Handle a compacted agent: re-inject current task context."""
    if agent_def.get("type") != "tmux":
        return False
