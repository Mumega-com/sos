...
def _detect_openclaw_state(agent_id: str, agent_def: dict) -> dict:
    ...
    try:
        ...
        if result.returncode != 0:
            ...
            unknown_agent = "Unknown agent id" in error
            cold_idle = agent_def.get("warm_policy", "cold") == "cold" and not _agent_has_active_tasks(agent_id)
            if unknown_agent and cold_idle:
                return {"state": "parked", "details": f"cold openclaw agent '{agent_id}' not configured in OpenClaw"}
            return {"state": "dead", "details": f"openclaw session query failed: {error[:100]}"}
...
def _snapshot_to_mirror(agent_id: str) -> bool:
    ...
    try:
        ...
        resp = requests.post(
            f"{MIRROR_URL}/store",
            json=payload,
            headers={"Authorization": f"Bearer {os.environ.get('MIRROR_TOKEN') or os.environ.get('MIRROR_ADMIN_TOKEN', '')}"},
            timeout=5,
        )
        if resp.status_code == 200:
