...
CUSTOMER_TOOLS: list[dict] = [
    {
        "name": "boot_context",
        "description": (
            "Show the current Mumega identity, selected project, agent, and "
            "memory boundary. Use this first when a model connects so it does "
            "not need to rediscover tenant context manually."
        ),
...
# These are always allowed regardless of role/tier so a fresh BYOA
