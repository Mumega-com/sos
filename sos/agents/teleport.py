...
async def teleport(config: TeleportConfig) -> TeleportResult:
    ...
    try:
        created, _ = append_if_missing(
            ...
            lambda t: t.get("project") == config.name,
        )
        if created:
    ...
    # Mint or reuse Mirror token through the shared DB-first provisioner.
    try:
        mirror_token, _mirror_minted = mint_or_get_mirror_key(
            config.name,
            f"{config.name} mirror access",
        )
        result.mirror_token = mirror_token
    except Exception as e:
        result.errors.append(f"Mirror key provisioning: {e}")
        mirror_token = ""
