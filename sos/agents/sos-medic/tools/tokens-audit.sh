...
grep -rlE "tokens\.json|bus/tokens" --include='*.py' sos/ /home/sos/mirror/ 2>/dev/null | sort -u

echo
echo "=== suspicious anti-pattern occurrences ==="
grep -rnE 'entry\.get\("token"\)\s*==|entry\["token"\]\s*==|x\.get\("token"\)\s*==' \
    --include='*.py' sos/ /home/sos/mirror/ 2>/dev/null || echo "  (none found — clean)"

echo
echo "=== hash-aware verifiers (known-good pattern) ==="
grep -rnE 'token_hash|hashlib\.sha256\(token' --include='*.py' sos/ /home/sos/mirror/ 2>/dev/null | head -20
