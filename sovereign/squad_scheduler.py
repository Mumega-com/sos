...
MIRROR_AUTH = f"Bearer {os.environ.get('MIRROR_TOKEN') or os.environ.get('MIRROR_ADMIN_TOKEN', '')}"
ORGANISMS_DIR = Path("/home/sos/.mumega/organisms")
BUDGETS_DIR = Path("/home/sos/SOS/sovereign/.budgets")
ENGAGEMENTS_FILE = Path("/home/sos/SOS/sovereign/.budgets/engagements.json")
...
def _get_redis():
    ...
    if not password:
        env_path = Path("/home/sos/.env.secrets")
        if env_path.exists():
