...
- Bus watcher safety MVP: JSON config, durable delivered-message state,
  command allowlist, token redaction, launchd plist generation, and
  "no mark-seen until transport exits 0" delivery semantics.
- Safer bus watcher install token sources: `SOS_BUS_TOKEN`/`--token-env`,
  `--token-file`, and `--token-stdin`, so real installs do not need bearer
  tokens in process arguments or shell history.
- `mumega-bus-watch install` now stores `token_file`/`token_env` references
  instead of copying bearer values into config, and `status` reports only the
...
- SDK `Agent.inbox()` can read through `bridge_url`/`SOS_BRIDGE_URL` for
  remote agents that do not have local Redis.
- Bus bridge `/inbox` now includes authorized subscription streams, including
