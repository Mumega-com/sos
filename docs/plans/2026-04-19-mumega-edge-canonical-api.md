...
| Repo | Change |
|------|--------|
| `/home/sos/mumega-edge/` (this Worker) | Add `/sos/*` + `/inkwell/*` route groups, keep existing auth |
| `/home/sos/inkwell/` (template) | `workerUrl` default = `https://api.mumega.com` |
...
Edit `/home/sos/inkwell/inkwell.config.ts`:
