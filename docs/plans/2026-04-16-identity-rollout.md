...
## Tier B — coordinator tmux under `/home/sos`, not currently driving work

| Agent | tmux session | Home |
|---|---|---|
| `athena` | athena | `/home/sos` |
| `mumega` | mumega | `/home/sos` |
| `mumega-web` | mumega-web | `/home/sos` |
| `mumega-com-web` | mumega-com-web | `/home/sos` |
| `prefrontal` | prefrontal | `/home/sos` (tmux cwd) — but the agent itself runs as Linux user `prefrontal`, handled via `sos.cli.onboard` separately; LEAVE ALONE |
| `gemini` | gemini | `/home/sos` |
| `river` | river | `/home/sos` (alias session for gemini) |

**Problem:** they all share `/home/sos/.claude.json` / `~/.claude/settings.json`, so they all collapse to kasra identity. But each tmux needs its OWN home directory for the `.mcp.json` to take effect (Claude Code only picks up `.mcp.json` from cwd or parent dirs).
...
|---|---|---|---|
| `kasra` | kasra | `/home/sos` | **currently attached — Hadi is using it right now** |
| `codex` | codex | `/home/sos` | idle but authoritative for infra work |
| `mumcp` | mumcp | `/home/sos/projects/sitepilotai` | already has project-local cwd |
...
| Agent | Home | Status |
|---|---|---|
| `trop` | `/home/sos/therealmofpatterns` | Needs `.mcp.json` with trop's existing `sk-trop-*` token, not a fresh mint — provisioner needs a `--reuse-token` flag first |
