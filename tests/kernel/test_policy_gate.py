...
async def test_scoped_service_token_allows_with_service_scope_not_admin(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    """Narrow env service tokens pass as service scope, not break-glass admin."""
    import sos.kernel.audit as audit
    monkeypatch.setattr(audit, "_audit_dir", lambda: tmp_path / "audit")
    monkeypatch.setenv("SOS_SQUAD_TOKEN", "squad-123")

    decision = await can_execute(
        action="task_create",
        resource="task",
        tenant="othertenant",
        authorization="Bearer squad-123",
    )

    assert decision.allowed is True
    assert "service scope" in decision.reason
    assert "system/admin" not in decision.reason
    assert "service_scope" in decision.pillars_passed
    assert "system_admin" not in decision.pillars_passed
