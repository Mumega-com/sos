...
def _clean_env(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    for var in (
        ...
        "SOS_SQUAD_TOKEN",
        "SOS_BILLING_TOKEN",
        "SOS_INTEGRATIONS_TOKEN",
        "SOS_HEALTH_TOKEN",
        "SOS_MATCHMAKER_TOKEN",
        "SOS_OPERATIONS_TOKEN",
        "SOS_JOURNEYS_TOKEN",
        "SOS_IDENTITY_TOKEN",
        "SOS_OBJECTIVES_TOKEN",
        "SOS_ECONOMY_TOKEN",
        "SOS_GLASS_SYSTEM_TOKEN",
        "SOS_MEMORY_TOKEN",
    ):
...
def test_verify_bearer_squad_token_is_scoped_not_admin(
    auth_module, monkeypatch: pytest.MonkeyPatch
):
    auth, _ = auth_module
    monkeypatch.setenv("SOS_SQUAD_TOKEN", "squad-secret-42")
    ctx = auth.verify_bearer("Bearer squad-secret-42")
    assert ctx is not None
    assert ctx.is_system is True
    assert ctx.is_admin is False
    assert ctx.env_source == "SOS_SQUAD_TOKEN"
    assert auth.has_scope(ctx, "squad:write") is True
    assert auth.has_scope(ctx, "registry:write") is False
