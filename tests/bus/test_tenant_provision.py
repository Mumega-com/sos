...
def tmp_substrate(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("MIRROR_URL", raising=False)
...
class TestMirrorKeyIdempotent:
    def test_fresh_mint_creates_file(self, tmp_substrate):
        ...
        assert data[0]["source"] == "legacy_file"

    def test_db_issued_mirror_token_cached_for_delivery(self, tmp_substrate, monkeypatch):
        monkeypatch.setattr(tp, "_issue_db_mirror_token", lambda slug, display_name: {
            "key": "sk-acme-db-issued",
            "mirror_workspace_id": "ws-db-acme",
            "mirror_token_id": "tok-db-acme",
        ...
        assert minted is True
        assert key == "sk-acme-db-issued"
        data = json.loads(tmp_substrate["mirror_keys_path"].read_text())
        assert data[0]["source"] == "mirror_tokens"
        assert data[0]["mirror_workspace_id"] == "ws-db-acme"
        assert data[0]["mirror_token_id"] == "tok-db-acme"

    def test_existing_db_issued_cache_prevents_remint(self, tmp_substrate, monkeypatch):
        calls = {"count": 0}

        def issue(slug, display_name):
            calls["count"] += 1
            return {
                "key": "sk-acme-db-issued",
                "mirror_workspace_id": "ws-db-acme",
        ...
        monkeypatch.setattr(tp, "_issue_db_mirror_token", issue)

        key1, minted1 = tp.mint_or_get_mirror_key("acme", "Acme Corp")
        key2, minted2 = tp.mint_or_get_mirror_key("acme", "Acme Corp")

        assert (key1, minted1) == ("sk-acme-db-issued", True)
        assert (key2, minted2) == ("sk-acme-db-issued", False)
        assert calls["count"] == 1

    def test_existing_db_issued_cache_verified_active_when_admin_configured(self, tmp_substrate, monkeypatch):
        seed = [{
            "key": "sk-acme-db-issued",
            "agent_slug": "acme",
        ...
        monkeypatch.setenv("MIRROR_ADMIN_TOKEN", "root-admin")

        def mirror_admin_request(method, path, token, body=None):
            assert (method, path, token, body) == (
                "GET",
                "/admin/workspaces/ws-db-acme/tokens",
                "root-admin",
                None,
            )
            return {"tokens": [{"id": "tok-db-acme", "token_type": "agent"}]}

        monkeypatch.setattr(tp, "_mirror_admin_request", mirror_admin_request)
        monkeypatch.setattr(tp, "_issue_db_mirror_token", lambda slug, display_name: pytest.fail("reminted active token"))

        key, minted = tp.mint_or_get_mirror_key("acme", "Acme Corp")

        assert (key, minted) == ("sk-acme-db-issued", False)

    def test_revoked_db_issued_cache_is_inactivated_and_reissued(self, tmp_substrate, monkeypatch):
        seed = [{
            "key": "sk-acme-old-db-issued",
            "agent_slug": "acme",
            "created_at": "2026-05-08T00:00:00+00:00",
            "active": True,
            "label": "Acme mirror access",
            "source": "mirror_tokens",
            "mirror_workspace_id": "ws-db-acme",
            "mirror_token_id": "tok-db-old",
        }]
        tmp_substrate["mirror_keys_path"].write_text(json.dumps(seed))
        monkeypatch.setenv("MIRROR_ADMIN_TOKEN", "root-admin")
        monkeypatch.setattr(tp, "_mirror_admin_request", lambda method, path, token, body=None: {"tokens": []})
        monkeypatch.setattr(tp, "_issue_db_mirror_token", lambda slug, display_name: {
            "key": "sk-acme-new-db-issued",
            "mirror_workspace_id": "ws-db-acme",
            "mirror_token_id": "tok-db-new",
            "source": "mirror_tokens",
        })

        key, minted = tp.mint_or_get_mirror_key("acme", "Acme Corp")

        assert (key, minted) == ("sk-acme-new-db-issued", True)
        data = json.loads(tmp_substrate["mirror_keys_path"].read_text())
        assert len(data) == 2
        assert data[0]["active"] is False
        assert data[0]["inactive_reason"] == "mirror_db_token_inactive"
        assert data[1]["active"] is True
        assert data[1]["mirror_token_id"] == "tok-db-new"
