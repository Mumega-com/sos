...
class _FakeRedis:
    ...
    def xrange(self, stream, min="-", max="+", count=10):
...
def test_inbox_reads_authorized_subscription_streams(
    monkeypatch, fake_redis, token_kasra
):
    token = {
        **token_kasra,
    ...
    _patch_token_resolution(monkeypatch, token)
    fake_redis.streams["sos:stream:project:sos:broadcast"] = [
        (
            "1778811511313-0",
            {
                "source": "agent:loom",
                ...
                "type": "broadcast",
                "payload": json.dumps({"text": "project broadcast"}),
    ...
    h = _make_handler(
        method="GET",
        path="/inbox?agent=kasra&project=sos&limit=10&format=json&since=1778811511312-0",
        body=None,
        token=token,
    )
    ...
    assert 200 in h._captured_status  # type: ignore[attr-defined]
    body = _last_response_body(h)
    assert [message["text"] for message in body["messages"]] == ["project broadcast"]
    assert body["messages"][0]["stream"] == "sos:stream:project:sos:broadcast"
    assert body["messages"][0]["stream_kind"] == "subscription:sos:channel:project:sos:global"


def test_inbox_runtime_subscription_override_is_project_scoped(
    monkeypatch, fake_redis, token_kasra
):
    token = {
        **token_kasra,
    ...
    fake_redis.streams["sos:stream:project:sos:broadcast"] = [
        (
            "1778811511313-0",
            {
                ...
                "target": "sos:channel:project:sos:global",
                "type": "broadcast",
                "payload": json.dumps({"text": "runtime project broadcast"}),
                "timestamp": "2026-05-15T02:20:00+00:00",
                "project": "sos",
            },
        )
    ...
    h = _make_handler(
        method="GET",
        path=(
            "/inbox?agent=kasra&project=sos&limit=10&format=json"
            "&subscriptions=sos:channel:project:sos:global"
        ),
        body=None,
        token=token,
    ...
    assert 200 in h._captured_status  # type: ignore[attr-defined]
    body = _last_response_body(h)
    assert [message["text"] for message in body["messages"]] == ["runtime project broadcast"]
    assert body["messages"][0]["stream"] == "sos:stream:project:sos:broadcast"


def test_watch_cross_agent_read_returns_403_before_sse(
    monkeypatch, fake_redis, token_kasra
):
    _patch_token_resolution(monkeypatch, token_kasra)
    h = _make_handler(
        method="GET", path="/watch?agent=loom&limit=10",
        body=None, token=token_kasra,
    )
...
def test_watch_authorized_path_delegates_with_initial_cursor(
    monkeypatch, fake_redis, token_kasra
):
    captured = {}
    _patch_token_resolution(monkeypatch, token_kasra)

    def _capture_watch(self, **kwargs):
        captured.update(kwargs)
        self._json(200, {"watch": "captured"})

    monkeypatch.setattr(BusHandler, "_handle_watch", _capture_watch)
    h = _make_handler(
        method="GET",
        path="/watch?agent=kasra&project=sos&limit=7&since=1778811511313-0",
        body=None,
        token=token_kasra,
    )
    ...
    assert 200 in h._captured_status  # type: ignore[attr-defined]
    assert captured["agent"] == "kasra"
    assert captured["project"] == "sos"
    assert captured["since"] == "1778811511313-0"
    assert captured["limit"] == 7


def test_inbox_helper_can_return_watch_batches_oldest_first(
    monkeypatch, fake_redis, token_kasra
):
    h = BusHandler.__new__(BusHandler)
    fake_redis.streams["sos:stream:project:sos:agent:kasra"] = [
        (
            "1778811511313-0",
            {
                "source": "agent:loom",
        ...
        (
            "1778811511314-0",
            {
                ...
                "target": "agent:kasra",
                "type": "send",
                "payload": json.dumps({"text": "second"}),
                "timestamp": "2026-05-15T02:21:00+00:00",
                "project": "sos",
            },
        ),
    ]

    messages = h._read_inbox_messages(
        token={**token_kasra, "project": "sos"},
        agent="kasra",
        project="sos",
        since=None,
        limit=10,
...
def test_project_broadcast_targets_project_global_channel(
    monkeypatch, fake_redis, token_kasra
):
    token = {
        **token_kasra,
    ...
    h = _make_handler(
        method="POST",
        path="/broadcast",
        body={"from": "kasra", "project": "sos", "text": "project broadcast"},
        token=token,
    )
    ...
    assert 200 in h._captured_status  # type: ignore[attr-defined]
    body = _last_response_body(h)
    assert body["channel"] == "sos:channel:project:sos:global"
    stream_rows = fake_redis.streams["sos:stream:project:sos:broadcast"]
    assert stream_rows[0]["target"] == "sos:channel:project:sos:global"
