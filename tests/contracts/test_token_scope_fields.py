...
def test_scopes_field_in_tokens_json_populates_auth_context(tmp_path, monkeypatch):
    """A tokens.json entry with scopes must preserve them in AuthContext."""
    from sos.kernel.auth import has_scope, verify_bearer

    _write_tokens(
        tmp_path,
        monkeypatch,
        [{"label": "svc", "token": "tk_svc", "active": True, "scopes": ["registry:*"]}],
    )

    ctx = verify_bearer("Bearer tk_svc")
    assert ctx is not None
    assert ctx.scopes == ["registry:*"]
    assert has_scope(ctx, "registry:read") is True
    assert has_scope(ctx, "squad:read") is False
